...
The above evaluation statistics results suggested that different selection methods have their own specific optimal size. Now we simply selected top 45% genes based on their scores (except for hvg and **DUBStepR**), which requires further adaption of the feature sizes. 
...
#### new PCA, cell type
```{r pc, warning=FALSE, fig.height=10, fig.width=8}
pc <- lapply(names(sces), \(x) plotPCA(sces[[x]], colour_by = "cluster_id") + 
    ggtitle(x)) |> 
    wrap_plots(ncol = 2) + plot_layout(guides = "collect")
pc
```
