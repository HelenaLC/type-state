         ...
         base = 2) 
{
  ...
  # when cluster size is small, a gene is likely to only express in one cluster
  res[is.na(res)] <- 0
  ts <- c(by(res, res$marker_id, \(.) mean(1-.$entropy)))
  return(ts)
}