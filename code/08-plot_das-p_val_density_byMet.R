...
p1 <- lapply(unique(all$das), \(s){
    df <- all[all$das == s,]
    gg <- ggplot(df, aes(p_val, after_stat(ndensity), col = sel)) +
...
p2 <- lapply(unique(out$dat), \(s){
    fd <- out[out$dat == s,]
    p <- ggplot(fd, aes(p_val, after_stat(ndensity), col = sel)) +
        geom_density(key_glyph = "smooth") +
        facet_wrap(~ das, ncol = 3, scales = "free") + 
        scale_x_continuous("p-value", breaks = seq(0, 1, 0.5), expand = c(0, 0.05)) +
        scale_y_continuous("normalized density", breaks = c(0, 1), expand = c(0, 0.1)) +
        ggtitle(s) + 
        theme_bw(9) + theme(
