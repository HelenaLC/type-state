...
p1 <- lapply(names(df_by_sel), \(sel) {
    fd <- df_by_sel[[sel]]
    fd_by_das <- split(fd, fd$das)
    gs <- lapply(fd_by_das, \(.) .$gene_id[.$p_adj])
    # plotting
    upset(fromList(gs), rev(DAS), sort_sets=FALSE,
        height_ratio=1, width_ratio=1/4, 
        name="", sort_intersections_by="degree",
        ...
        set_sizes=upset_set_size(position="right") + 
            ...
            scale_y_continuous(NULL, limits=c(0, 2e3), n.breaks=2),
        base_annotations=list(
            "Intersection size"=
                intersection_size(counts=FALSE) +
                scale_y_continuous(NULL,limits=c(0, 800), n.breaks=3) +
                ggtitle(sel) + theme(
...
p2 <- lapply(names(df_by_das), \(das) {
    fd <- df_by_das[[das]]
    fd_by_sel <- split(fd, fd$sel)
    gs <- lapply(fd_by_sel, \(.) .$gene_id[.$p_adj])
    # plotting
    upset(fromList(gs), rev(SEL), sort_sets=FALSE, name="", 
        height_ratio=1, width_ratio=1/4,
        ...
        base_annotations=list(
            "Intersection size"=
                intersection_size(counts=FALSE) +
                scale_y_continuous(NULL, limits=c(0, 500), n.breaks=3) +
                ggtitle(das) + theme(
                    legend.position="none", 
                    panel.grid.minor=element_blank(),
                    panel.grid.major=element_line(linewidth=0.2),
